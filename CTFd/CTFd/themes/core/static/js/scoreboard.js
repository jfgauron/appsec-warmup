    ...
    $.get(script_root + '/api/v1/scoreboard', function (response) {
        ...
        for (var i = 0; i < teams.length; i++) {
            var row = "<tr>\n" +
                ...
                "<td><a href=\"{0}/users/{1}\">{2}</a></td>".format(script_root, teams[i].account_id, htmlentities(teams[i].name)) +
...
function scoregraph () {
    $.get(script_root + '/api/v1/scoreboard/top/10', function( response ) {
        ...
        if (Object.keys(places).length === 0 ){
            // Replace spinner
            $('#score-graph').html(
                '<div class="text-center"><h3 class="spinner-error">Aucune solution encore</h3></div>'
            );
            return;
        }
...
setInterval(update, 15000); // Update scores every 15 seconds
scoregraph();

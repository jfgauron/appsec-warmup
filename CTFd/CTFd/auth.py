...
def confirm(data=None):
    ...
    if data and request.method == "GET":
        try:
            ...
            return render_template('confirm.html', errors=['Votre lien de confirmation a expiré'])
        except (BadSignature, TypeError, base64.binascii.Error):
            return render_template('confirm.html', errors=['Votre token de confirmation est invalide'])
    ...
    if data is None:
        if request.method == "POST":
            ...
            return render_template('confirm.html', user=user, infos=['Votre courriel de confirmation a été renvoyé!'])
    ...
    if request.method == 'POST':
        ...
        if user:
            if user and check_password(request.form['password'], user.password):
                ...
                errors.append("Votre nom d\'utilisateur ou mot de passe est incorrect")
                db.session.close()
